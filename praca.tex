...
ECMarket ma na chwilę obecną kilka wartych uwagi konkurentów, którzy nie dorównują naszemu rozwiązaniu w paru kwestiach.
\begin{itemize}
	\item \textit{uPort} operuje na własności tożsamości, ze względu na cele tej aplikacji, czyli handel tożsamością, anonimowość nie jest dostępna.
	\item \textit{Ujo Music} handluje tylko utworami muzycznymi, nie posiada również rozbudowanego systemu konktraktów.
	\item \textit{IDEX} oferuje tylko usługi wymiany walut.
	\item \textit{OpenBazaar} w swojej przystępności nie oferuje możliwości stworzenia i podpisania kontraktu z drugą stroną, a co za tym idzie, bezpieczeństwo transakcji jest bardzo zależne od jednej ze stron. 
\end{itemize}
Mimo silnej konkurencji na nowym, chłonnym rynku o nowej technologii, ECMarket ma zalety i możliwości, aby przerosnąć konkurencję.
