...
instytucji państwa \cite{website:trustPOL,website:trustUSA} jak i korporacji, powstało wiele projektów mających
...
\subsection{Analiza Przyrostu}
Na samym początku zabraliśmy się za analizę naszego systemu. 
\subsection{Projektowanie Przyrostu}
...
\subsection{Cele przyrostu}
Drugi przyrost miał na celu przeanalizowanie, w jaki sposób powinny wyglądać kontrakty wykorzystywane przez użytkowników w naszym systemie oraz implementację jednego z nich i  migrację tokenów systemowych do formatu zgodnego z ERC20. Ponadto zaczęliśmy analizować rozwiązania i sposoby na zachęcenie użytkowników do usuwania przestarzałych lub zakończonych umów, aby zwiększyć efektywność i produktywność naszego systemu. Przyrost posłużył nam również jako podstawa do przygotowania serwisów. Postawiliśmy sobie za cel rozbudowanie  routing'u we wszystkich komponentach jak i poprawę interfejsu użytkownika. Ostatnim zadaniem przyrostu było stworzenie interakcji z kontraktami.

\subsection{Analiza Przyrostu}
Postanowiliśmy, że naszym głównym przedmiotem badań będzie przeanalizowanie sprawiedliwych kontraktów. Podczas analizowania różnych przypadków postanowiliśmy że system powinien dostarczać różne rodzaje kontraktów w zależności od potrzeb użytkowników. Podczas prac analizowaliśmy różnego przypadki umów, na koniec postanowiliśmy że w ramach naszej pracy inżynierskiej stworzymy dwa typy kontraktów.
...
Umowa przedstawiona powyżej jest najprostszą dostępną umową w naszym systemie. Umowę rozpoczyna Sprzedawca, który tworzy umowę (punkt 0 na rysunku \ref{A1_1}) . Potencjalny kupujący zainteresowany umową wpłaca pieniądze(1), a w następnym kroku Sprzedawca potwierdza transakcję i wypłaca przedmiot zakupu(2). Na koniec kupujący ocenia sprzedającego(3). W umowie istnieją alternatywne scenariusze, ponieważ kupujący może nie chcieć zawrzeć umowy z nisko ocenianym sprzedającym i odrzucić ofertę(2a), gdzie następnym krokiem jest wypłacenie przez kupującego(3a). Jeżeli dojdzie do porozumienia lub też nie, umowa zostaje zakończona(4a).
...
\subsection{Cele przyrostu}
\subsection{Projektowanie Przyrpstu}
\subsection{Testy walidacyjne}
\subsection{Testy integracyjne}
