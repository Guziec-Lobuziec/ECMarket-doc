...
\begin{itemize}
	...
	\item Oliver Śliwonik -Basista: \begin{itemize}
		...
		\item wspótworzenie testów do umów
	\end{itemize}
...
Oliver Śliwonik -Basista:
\begin{itemize}
		\item analizowałem ograniczenia i własność platformy Ethereum, w jaki sposób gwarantuje zachowanie swoich kluczowych atrybutów oraz konstrukcje są możliwe do zaimplementowania na tej platformie
		
		\item projektowałem i analizowałem architekturę naszego rozwiązania
		
		\item przeprowadziłem rozpoznanie problemu potencjalnej centralizacji Ethereum, porównałem jak ta kwestia wygląda w przypadku innych kryptowalut, zdiagnozowałem jakie czynniki mają istotny wpływ na to zjawisko oraz jakie rozwiązania tego problemu obecnie istnieją
		
		\item przygotowałem zestawienie znanych ataków na kontrakty Ethereum oraz przeprowadziłem analizę, jak możemy zapobiegać ich wystąpieniu w naszej pracy
		
		\item zdiagnozowałem problem braku możliwości agregacji lub kompozycji kontraktów
		
		\item zdiagnozowałem wpływ modelu pamięci \textit{storage} w języku Solidity na sposób projektowania systemów w Ethereum
		
		\item przygotowałem rozwiązanie problemu braku kompozycji i agregacji kontraktów w postaci autorskiej architektury maszyny stanowej
		
		\item przygotowałem rozszerzenie modelu pamięci \textit{storage} w języku Solidity, przez skonstruowanie abstrakcji pozwalającej do niej pisać jak do pliku.
		
		\item zaimplementowałem podsystem zarządzania umowami oraz przygotowałem zestaw testów do niego
		
		\item przygotowałem testy określające przebieg wykonania umowy A1.1
		
		\item wspierałem implementację routingu i serwisów po stronie front-endu
		
		\item przygotowałem zestawy ćwiczeń i materiałów, pomagających zapoznać się z Solidity i Ethereum
		
		\item wprowadzałem do naszego projektu standardy wykorzystywane przez społeczność Ethereum, np.\ ERC20
		
		\item przygotowałem streszczenie opisu działania Ethereum
\end{itemize}
