...
Przy tworzeniu systemu moim zadaniem była przede wszystkim implementacja umowy A1.1 oraz przetestowanie jej. Wspomagałem również Olivera przy tworzeniu podsystemu do zarządzania umowami, w którym tworzyłem różne właściwości dla umów tj. data ważności czy też kontrola dołączania do umowy przez różne osoby. Stworzyłem również implementację Virtual Wallet oraz przetestowałem go, jednak  wraz z ewolucją całego projektu oraz napotkanymi problemami, implementacja całego Virtual Wallet została usunięta z tego wydania.

W ramach tworzenia interfejsu graficznego, moim zadaniem było stworzenie komponentu Overview gdzie użytkownik ma podgląd na umowy które stworzył oraz te umowy do których dołączyć jako kupujący. Moim zadaniem również było stworzenie serwisu do wyszukiwania umów oraz testów do tego serwisu, który po wpisaniu odpowiedniego parametru wyszukiwał umowę. Brałem również udział w projektowaniu komponentu do wyszukiwania.
