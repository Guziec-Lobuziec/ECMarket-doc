...
\begin{itemize}
	...
	\item Oliver Śliwonik -Basista: \begin{itemize}
		...
		\item omówienie problemu centralizacji Ethereum oraz innych kryptowalut, czynników wpływających na to zjawisko, jak również potencjalnych rozwiązań tej kwestii
		...
		\item implementacja podsystemu zarządzającego umowami
		\item współtworzenie testów do umów
	\end{itemize}
	...
	\item Bartosz Rychlewski: \begin{itemize}
		...
		\item udokumentowanie wszystkich tabelek, drugiego przyrostu, proponowanego rozwiązania, planu zapewnienia jakości, infrastruktury komunikacyjnej i dokumentacyjnej, opisu klas, raportu końcowego
...
\begin{table}[H]
	...
	\textit{\textbf{Metody}} & \multicolumn{2}{m{12cm}|}{\textit{\textbf{fallback()} --- przechwytuje wszystkie wywołania i przekazuje je do kodu odpowiedzialnego za obecny stan }}    \\ \hline
\end{tabular}
\end{table}
\label{OMclass6}
\end{figure}
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Wspólny interfejs dla stanów i maszyny }} \\ \hline
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Stan początkowy umowy. Umowa jest w nim zaraz po utworzeniu. }} \\ \hline
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Klasa abstrakcyjna zbierająca część implementacji.}} \\ \hline
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Stan umowy po zaakceptowaniu jednego z petentów.}} \\ \hline
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Końcowy stan umowy, w którym oczekuje na usunięcie.}} \\ \hline
...
\caption{Klasa abstrakcyjna}
\begin{table}[H]
    \begin{tabular}{|l|l|l|l|}
    \hline
    \textit{\textbf{Klasa}}                & \multicolumn{2}{m{12cm}|}{\textit{StateCommons}}                                    \\ \hline
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Klasa abstrakcyjna ze wspólną częścią implementacji dla stanów.}} \\ \hline
    \multirow{ 2}{*}{\textit{\textbf{Metody}}} & \multicolumn{2}{m{12cm}|}{\textit{\textbf{withdraw()} --- pozwala wypłacić tokeny}}    \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{expirationSensitive()} --- prywatny modyfikator}} \\ 
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Biblioteka dostarczająca abstrakcję nad modelem pamięci solidity}} \\ \hline
    \textit{\textbf{Zmienne}} & \multicolumn{2}{m{12cm}|}{\textit{\textbf{SPOINTER\_SIZE} ---  ustawia wielkość wskaźnika}}    \\ \hline
    \multirow{ 2}{*}{\textit{\textbf{Metody}}} & \multicolumn{2}{m{12cm}|}{\textit{\textbf{setSlots()} ---  ustawia slot pamięci storage}}    \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getSlots()} --- odczytuje slot pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{setBytes32()} --- zapisuje do pamięci storage byte32}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getBytes32()} --- odczytuje z pamięci storage byte32}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{setBytes()} --- zapisuje tablicę byte'ów do pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getBytes()} --- odczytuje tablicę byte'ów z pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{setMapping()} ---zapisuje element mappingu do pamięci storage }} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getMapping()} --- pobiera element mappingu z pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{setStoragePointer()} --- zapisuje wybrany wskaźnik do pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getStoragePointer()} --- pobiera wybrany wskaźnik z pamięci storage}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getStoragePointerMapping()} --- pobiera wskaźnik na mapping }} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{setPositionAt()} --- ustawia wskaźnik na wybranym slocie}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{mapSPointerTo()} --- pobiera numer obecnego slotu, hashuje go i ustawia w to miejsce wskaźnik}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{relativeMove()} --- przesuwa wskaźnik relatywnie do jego pozycji {\fontsize{5pt}{5.5cm}\selectfont :)}}} \\ 
		&  \multicolumn{2}{m{12cm}|}{\textit{\textbf{getAbsoluteSlotLocation()} --- zwraca numer slotu}} \\ \hline
\end{tabular}
\end{table}
\label{OMclass15}
\end{figure}
...
\begin{table}[H]
    ...
    \textit{\textbf{Opis klasy}}                 & \multicolumn{2}{m{12cm}|}{\textit{Część mechanizmu abstrakcji nad pamięcią storage, jest wykorzystywana przez stan umowy do utrwalania danych}} \\ \hline
    \textit{\textbf{Zmienne}} & \multicolumn{2}{m{12cm}|}{\textit{\textbf{storageManagers} --- prywatne pole}}    \\ \hline
...
W ramach projektu zespołowego razem z całym zespołem tworzyłem i analizowałem różne rodzaje umów, które potencjalnie możemy wykorzystać w naszym systemie. \\

W ramach pisania pracy tworzyłem diagramy klas dla back-endu oraz front-endu oraz diagram sekwencji dla wyszukiwania umów. Byłem współodpowiedzialny za stworzenie harmonogramu prac , instrukcji obsługi oraz tematu związanego z konkurencją. Moim zadaniem było również rozpisanie przyrostów. \\

Przy tworzeniu systemu moim zadaniem była przede wszystkim implementacja umowy A1.1 oraz przetestowanie jej. W umowie  A1.1 znalazły się podstawowe funkcje potrzebne do przeprowadzenia całego procesu transakcji. Wspomagałem również Olivera przy tworzeniu podsystemu do zarządzania umowami, w którym tworzyłem różne właściwości dla umów tj. data ważności czy też kontrola dołączania do umowy przez różne osoby. Stworzyłem również implementację Virtual Wallet oraz przetestowałem go. W Virtual Wallet tworzyłem funkcję do  wpłacania oraz wypłacania tokenów czy też sprawdzenia balansu, jednak  wraz z ewolucją całego projektu oraz napotkanymi problemami, implementacja całego Virtual Wallet została przerobiona na standard ERC20. \\

W ramach tworzenia interfejsu graficznego, moim zadaniem było stworzenie komponentu Overview gdzie użytkownik ma podgląd na umowy, które stworzył oraz te umowy, do których dołączył jako kupujący. Moim zadaniem również było stworzenie serwisu do wyszukiwania umów oraz testów do tego serwisu, który po wpisaniu odpowiedniego parametru wyszukiwał umowę. Częścią implementacji serwisu do wyszukiwania umów, było stworzenie filtrów dla parametrów tj. cena, nazwa czy data. Filtry są wybierane w zależności od danych wejściowych użytkownika. Brałem również udział w projektowaniu komponentu do wyszukiwania umów. Rozbudowywałem również routing stworzony przez Bartka o swoje komponenty.\\
...
\begin{itemize}
	...
	\item Gaz -- jednostka możliwości wykonania kodu. Jest kupowana przez wywołującego przed wywołaniem transakcji i istnieje tylko na czas jej trwania. Na obecną chwilę każda instrukcja w Ethereum zużywa niezerową ilość gazu.
