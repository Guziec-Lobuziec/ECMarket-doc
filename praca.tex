...
Pierwsze wydanie systemu dostarczy klientom zestaw gotowych umów i system ocen, który będzie dawać nam przewagę nad konkurencją w takich kwestiach jak:
\begin{itemize}
\item Wiarygodny system ocen --- stanowiący kluczową wartość oferowaną przez nasze rozwiązanie. W naszym systemie nie występuje administrator, dlatego też niezwykle istotne było stworzenie środowiska promującego uczciwość. Podobne rozwiązania możemy zaobserwować w przeróżnych serwisach aukcyjnych, gdzie stanowią one informację dla kupującego o tym, czy może ufać sprzedającemu. Nasze rozwiązanie pozwala na ocenienie drugiej strony umowy --- takiej oceny dokonuje zarówno kupujący, jak i sprzedający. Dodatkowo, ocenom tym zostają nadane odpowiednie wagi tak, aby zapobiec próbom wpłynięcia na system ocen np.\ poprzez wchodzenie w małe transakcje z dużą ilością kont powiązanych ze sprzedającym. Wagi przyznawane są na podstawie ocen posiadanych przez stronę wystawiającą takową ocenę, a także jej wiarygodności dla społeczności platformy wyrażoną poprzez oceny pozytywne oraz ogólną ilość ocen.
\item Niezależność od walut FIAT --- na platformie płatności odbywać się będą przy pomocy Ether'u (ETH) --- kryptowaluty, której wartość jest niezależna od każdego centralnego organu finansowego. Naszych środków nie da się zamrozić tak, jak np.\ w przypadku Peso Argentyńskiego (ARS) w 2001 roku podczas kryzysu finansowego. Nie grozi nam także hiperinflacja, pieniędzy nie da się po prostu dodrukować przez specyfikę pozyskiwania Ether'u (ETH). %jakis odnosnik do tego peso z 2001 roku? czy nie trzeba?
\item Niezależność od stron trzecich --- transakcje odbywają się w ramach zdecentralizowanej sieci Ethereum, gdzie nie występuje żaden administrator czy instytucja będąca w stanie zablokować bądź cofnąć naszą transakcję. Gwarantuje to poczucie bezpieczeństwa i niezależności wyróżniające nas na tle konkurencyjnych serwisów nieopartych na technologii blockchain, gdzie strona trzecia już występuje. Zwiększa to także bezpieczeństwo naszych danych osobowych podczas zachowania większej anonimowości.
% "niezależności wyróżniające nas na tle konkurencyjnych serwisów nieopartych na technologii blockchain, gdzie strona trzecia już występuje."
% to jest niepoprawne językowo, ale nie wiem jaki miał być sens, więc nie jestem w stanie poprawić.
\item Bezpieczne wykonanie umów --- ???

%pamiętajmy, że tu trzeba uzupełnić

\item Różnorodność umów i sposobów na przeprowadzenie transakcji --- Przygotowaliśmy 11 przykładowych modeli umów, z czego 2 znajdą się w pierwszym wydaniu systemu. Gwarantuje to dostosowanie systemu do potrzeb użytkowników. Dodatkowo ilość umów może się powiększyć wraz z powiększaniem się bazy użytkowników. 
\item Globalny zasięg ofert --- sieć Ethereum wiąże się z brakiem ograniczeń co do lokalizacji na Ziemi. Korzystać z niej można z dowolnego jej zakątku, wystarczy jedynie połączenie z internetem. Ether stanowi jedyny wymóg by korzystać z naszej platformy i może zostać nabyty przez każdego. Pozwala to na o wiele większą wszechstronność od większości platform do handlu ograniczonych jedynie do rynku lokalnego. Konkurować z nami pod tym względem mogliby jedynie giganci, tacy jak np.\ \textit{Amazon} czy \textit{eBay}.
\item Budowanie nowych rozwiązań --- unikalna cecha wyróżniająca nas na rynku. Jestemy otwarci na społeczność tworząca się wokół naszej platformy w znacznie szerszy sposób niż nie-blockchain'owa konkurencja. Po wypuszczeniu produktu żyłby on własnym życiem kontrolowany w całości przez społeczność posiadającą możliwość wprowadzania modyfikacji, które mogłyby zostać przez większość zaakceptowane, bądź odrzucone, co stanowi gwarancję dostosowania ECMarket'u do potrzeb potencjalnego użytkownika. Jednakże dodanie funkcjonalności związanych z tą cechą jest zaplanowane na przyszłe wydania.
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item funkcjonalności systemu muszą się nadawać do rozbicia na podfunkcjonalności
	...
	\begin{itemize}
		...
		\item nieduże przyrosty w małych odstępach czasowych
...
\subsection{Cele przyrostu}
Drugi przyrost to przede wszystkim implementacja pierwszej umowy A1.1 by sprawdzić czy nie znajdziemy żadnych problemów podczas tworzenia naszego systemu.
Następnym istotnym celem było stworzenie wewnętrzengo tokena który był o standard implementacji ERC20, który definiuje podstawowe funkcjonalnościm, jakie wspierdają przez token.
Tworzony system jest bardzo nowym i eksperymentalnym doświadczeniem, dlatego naszym zadaniem w danym przyroscie był interfejs użytkownika, stworzony w podobieństwie do stadnardowych rozwiązań.
Zadaniem który sobie postawiliśmy na ten przyrost było stworzenie komponentów, gdzie znajdziemy wyszukiwanie umów, widok umów, tworzenie umów czy też przeglądanie akcji danego adresu.

\subsection{Analiza front-end'u}
\subsection{Projektowanie front-end'u}
\subsection{Testy}
\subsubsection{Środowisko testów}
Aplikacja było testowana za pomocą środowiska developerskiego truffle w którym testy tworzoyny za pomocą javascriptowego środkowska testowego Mocha oraz Chai do tworzenia assercji.
...
\begin{table}[H]
    \begin{tabular}{|l|l|}
\hline
    \textit{Umowa A1.1 Standardowa ściezka}                 & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy stworzenie umowy A1.1 z daną ceną w początkowym stanie. Użytkownik wpłaca pieniądze do kontraktu. Twórca wypłaca pieniądze. Na koniec każdy z użytkowników zamyka umowę. Umowa zostaje zakończona.}} \\ \hline
    \textit{Umowa A1.1 właściwości dołączania  :} & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy rózne scenariusze dołączania do umowy. Sprawdzamy czy umowa posiada do trzech użytkowników maksymalnie. Sprawdzamy również czy twórca czy suplikant nie może dołączyć dwa razy do tej samej umowy.}}    \\ \hline
    \textit{Umowa A1.1 ograniczenia akecptacji użytkowników: } & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy możliwości akceptacji użytkowników. Sprawdzamy czy twórca nie może akcpetowac samego siebie do umowy. Sprawdzamy czy tylko twórca może akceptowa innych do dołączenia do umowy. Sprawdzamy również czy nie możemy akcepotwa kogoś gdy umowa jest w trakcie realizacji.}}                                                \\ \hline
    \textit{Umowa A1.1 właściwości akceptacji zależne od stanu:} & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy czy nie możemy podwójnie zaakceptowac tej samej osoby. Sprawdzamy również czy nie można dołączyc do umowa gdy jest ona zamknięta.}}                                                \\ \hline
	\textit{Umowa A1.1 właściwości zamknięcia umowy:} & \multicolumn{1}{m{13cm}|}{\textit{Testujemy czy obce adresy nie mają możliwości zakończenia umowy. Sprawdzamy również czy niezaakceptowany adres nie może zamknąc umowy oraz czy nie możemy podówjnie zakończyć umowy.}} \\ \hline
	\textit{Interkacja umowy z funkcją remove: }    & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy czy różne funkcję takie jak join czy accept nie mają wpływu na usunięcie całego kontratktu. Sprawdzamy równiez czy nie możemy usunąć umowy gdy jest w stanie realizacji.}} \\ \hline
	\textit{Umowa A1.1 właściwości wycofania: }  & \multicolumn{1}{m{13cm}|}{\textit{W tym scenraiuszu testujemy wycofywanie się użytkownika z umowy. Sprawdzamy  czy sprzedawca nie może wycofac się z umowy. Wraz z wycofywaniem sprawdzamy czy jest możliwość wypłacenia tokenów. Sprawdzamy czy po staniu się kupującym nie może wycofac się z umowy.}}                                          \\ \hline
	\textit{AgreementManager tworzenie umów: }  & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy czy AgreementManager pomyślnie tworzy umowę. Sprawdzamy również czy umowa istnieje pod danym adresem.}}                                          \\ \hline
	\textit{AgreementManager właściwości wyszukiwania: }  & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy właściwości wyszukiwania AgreementManagera. Sprawdzamy czy AgreementManager pomyślnie wyszukuje pomyślnie daną umowę.}}                                          \\ \hline
\end{tabular}
\end{table}
...
\subsection{Cele przyrostu}
W trzecim przyroście za zadanie postawiliśmy sobie stworzenie wewnętrzengo Tokena w standradzie
obowiązującym w środowisku Ethereum (ERC20).Głównym zadaniem było jednak stworzenie po stronie interfejsu wygodnego wyszukiwania umów,
różnych widoków potrzebnych do tworzenia czy też przegląania umów.
\subsection{Napotkane problemy}
Na tym etapie podczas tworzenia implementacji umów, napotkaliśmy problem natury technologicznej.
Każda umowa którą zaprojektowaliśmy ma różne wspólne cechy. W języku programowania w którym pracujemy nie jesteśmy w stanie uogólni tego w żadnej sposób 
np. za pomocą dziedziczenia. Kontratky nie wspierają agregacji. Kolejnym problem również był samo tworzenie kontraktów jak standardowych obiektów który jest kosztowny, ponieważ każde
stworzenie takiego kontraktu wymaga użycia gazu.  
\subsection{Projektowanie back-end'u}
\subsection{Projektowanie front-end'u}
\subsection{Testy walidacyjne}
...
\begin{table}[H]
    \begin{tabular}{|l|l|}
\hline
    \textit{Standard ECM Token transfer: }                 & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy różne scenariusze przepływu tokenów pomiędzy kontami. Sprawdzamy poprawność transakcji.Sprawdzamy również czy nie możemy wysyłać tokentów do adresu danego tokenu}} \\ \hline
    \textit{Standard ECM Token wpłata oraz wypłata pieniędzy: } & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy poprawność wpłat i wypłat tokenów. Funkcje testowane są również na kilku użytkownikach oraz samego zdarzenia Transferu}}    \\ \hline
    \textit{Maszyna stanowa: } & \multicolumn{1}{m{13cm}|}{\textit{W tym scenariuszu sprawdzamy poprawaność przejścia stanu.}}                                                \\ \hline
    \textit{Maszyna stanowa z pamięcią:} & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy w tym scenariuszu poprawność zapisywania różnych stanów w pamięci}}                                                \\ \hline
	\textit{Obiekt Storage:} & \multicolumn{1}{m{13cm}|}{\textit{Testujemy inicjowanie obiektu Storage}} \\ \hline
    \textit{Zakres pamięci: }    & \multicolumn{1}{m{13cm}|}{\textit{Testujemy zachowanie pamięci poza zakresem.    }} \\ \hline
    \textit{Pamięc podstawy działania: }    & \multicolumn{1}{m{13cm}|}{\textit{Sprawdzamy poprawność funkcji których celem jest zapisywanie do danego slotu w pamięci.    }} \\ \hline
